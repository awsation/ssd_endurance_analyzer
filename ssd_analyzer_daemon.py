...
def run_smartctl(device: str) -> str:
    ...
    try:
        ...
        print(
            "Error: smartctl not found. Please install smartmontools.",
            file=sys.stderr
        )
        sys.exit(1)
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="SSD Endurance Analyzer Daemon"
    )
    parser.add_argument(
        '--device', required=True,
        help="Device to analyze (e.g., /dev/nvme0n1)"
    )
    parser.add_argument(
        '--host-lba-size', required=True, type=float,
        help='Host LBA size in KB'
    )
    parser.add_argument(
        '--flash-lba-size', required=True, type=float,
        help='Flash LBA size in KB'
    )
    parser.add_argument(
        '--rated-pe-cycles', required=True, type=int,
        help='Manufacturer rated P/E cycles'
    )
    parser.add_argument(
        '--capacity', type=float,
        help='Drive capacity in GB (optional)'
    )
    parser.add_argument(
        '--state-dir', type=Path,
        default=Path('/var/lib/ssd-analyzer'),
        help='Directory to store state (default: /var/lib/ssd-analyzer)'
    )
    parser.add_argument(
        '--log-dir', type=Path,
        default=Path('/var/log/ssd-analyzer'),
        help='Directory to store reports (default: /var/log/ssd-analyzer)'
    )
    return parser.parse_args()


def _resolve_capacity(args, snapshot2):
    """Resolve drive capacity from CLI arg or auto-detect from snapshot."""
    if args.capacity:
        return args.capacity
    if snapshot2.capacity_bytes:
        return snapshot2.capacity_bytes / (1024 ** 3)
    print(
        "Error: Could not auto-detect capacity. Please specify --capacity",
        file=sys.stderr
    )
    return None


def _generate_report(args, snapshot1, snapshot2, capacity_gb):
    """Calculate metrics and return the formatted report string."""
...
def main():
    """Entry point for the SSD endurance analyzer daemon."""
    args = _parse_args()
    ...
    capacity_gb = _resolve_capacity(args, snapshot2)
    ...
    report = _generate_report(args, snapshot1, snapshot2, capacity_gb)

    timestamp_str = (
        snapshot2.timestamp or datetime.datetime.now()
