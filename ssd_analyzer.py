...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Analyze SSD endurance and estimate lifetime from smartctl data',
...
Notes:
  ...
  - If capacity is not specified, it will be auto-detected from smartctl output
        """
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=int,
        help='Manufacturer rated P/E cycles (e.g., 3000 for TLC, 100000 for SLC)'
    )
...
def main():
    ...
    try:
        ...
        elif snapshot2.capacity_bytes:
            capacity_gb = snapshot2.capacity_bytes / (1024 ** 3)
        else:
            print("Error: Could not auto-detect capacity. Please specify --capacity",
                  file=sys.stderr)
            sys.exit(1)
        
        print(f"Drive capacity: {capacity_gb:.2f} GB", file=sys.stderr)
        ...
        # Calculate metrics
        print("Calculating endurance metrics...", file=sys.stderr)
